...
@PerkCode(PerkEnum.FRIENDLY_FOE)
@Component
public class FriendlyFoeRule implements PerkRule {
    @Override
    public boolean apply(Perk perk, Character character, Map<SpecialEnum, Integer> specialMap, Map<SkillEnum, Integer> skillValues, Integer rank) {
        return specialMap.getOrDefault(SpecialEnum.PERCEPTION, 0) >= 4 && character.getCurrentStats().getLevel() >= 6 && rank == 0;
    }
}
