...
@Component
public class BonusRateOfFireRule implements PerkRule {
    ...
    public boolean apply(Perk perk, Character character, Map<SpecialEnum, Integer> specialMap, Map<SkillEnum, Integer> skillValues, Integer rank) {
        return character.getCurrentStats().getLevel() >= 9
                && specialMap.getOrDefault(SpecialEnum.PERCEPTION, 0) >= 6
                && specialMap.getOrDefault(SpecialEnum.INTELLIGENCE, 0) >= 6
                && specialMap.getOrDefault(SpecialEnum.AGILITY, 0) >= 7
                && rank < 1;
    }
}
