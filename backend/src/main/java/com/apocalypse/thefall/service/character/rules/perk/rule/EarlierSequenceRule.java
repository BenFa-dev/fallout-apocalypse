...
@PerkCode(PerkEnum.EARLIER_SEQUENCE)
@Component
public class EarlierSequenceRule implements PerkRule {
    @Override
    public boolean apply(Perk perk, Character character, Map<SpecialEnum, Integer> specialMap, Map<SkillEnum, Integer> skillValues, Integer rank) {
        return specialMap.getOrDefault(SpecialEnum.PERCEPTION, 0) >= 6 && character.getCurrentStats().getLevel() >= 3 && rank < 3;
    }
}
