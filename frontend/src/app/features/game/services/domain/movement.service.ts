...
export class MovementService {
	...
	private newMove(dx: number, dy: number): Position | undefined {
		...
		if (!this.playerStore.canMove()) {
			this.notificationService.showNotification('warning', 'game.movement.errors.noActionPoints');
			return;
		}

		const newPosition: Position = {
			x: this.playerStore.playerPosition().x + dx,
			y: this.playerStore.playerPosition().y + dy
		};

		const targetTile = this.playerStore.currentTilesInVision().find((tile: Tile) =>
